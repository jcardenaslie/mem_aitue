{'readme': {'experiment_name': 'All Proyects Experimento base_minusisprofesion', 'experiment_dataset': 'personas_cotizacion6.csv', 'personas_shape': (6184, 36), 'personas_columns': ['Unnamed: 0', 'is_apellido1', 'is_appelido2', 'is_celular', 'is_direccion', 'is_fnac', 'is_nombre', 'is_nombrecompleto', 'is_nrofam', 'is_profesion', 'is_recontacto', 'is_telefono', 'loc_comuna', 'loc_provincia', 'loc_region', 'max_rango_edad', 'mean_cot_bod', 'mean_cot_depto', 'mean_cot_esta', 'mean_cot_estu', 'medio_inicial', 'negocio', 'nro_cot_bod', 'nro_cot_depto', 'nro_cot_esta', 'nro_cot_estu', 'nro_proyectos', 'profesion', 'rut', 'sexo', 'tipo_cliente', 'Altos del Valle', 'Edificio Urban 1470', 'San Andres Del Valle', 'Edificio Mil610', 'Edificio Junge'], 'personas_after_drop_columns': Index(['is_apellido1', 'is_appelido2', 'is_celular', 'is_direccion', 'is_fnac',
       'is_nombre', 'is_nombrecompleto', 'is_nrofam', 'is_telefono',
       'loc_comuna', 'loc_provincia', 'loc_region', 'mean_cot_bod',
       'mean_cot_depto', 'mean_cot_esta', 'mean_cot_estu', 'nro_cot_bod',
       'nro_cot_depto', 'nro_cot_esta', 'nro_cot_estu', 'nro_proyectos',
       'sexo', 'tipo_cliente', 'Altos del Valle', 'Edificio Urban 1470',
       'San Andres Del Valle', 'Edificio Mil610', 'Edificio Junge',
       'profesion'],
      dtype='object'), 'personas_predictors_shape': (6184, 29), 'personas_predictors': Index(['is_apellido1', 'is_appelido2', 'is_celular', 'is_direccion', 'is_fnac',
       'is_nombre', 'is_nombrecompleto', 'is_nrofam', 'is_telefono',
       'loc_comuna', 'loc_provincia', 'loc_region', 'mean_cot_bod',
       'mean_cot_depto', 'mean_cot_esta', 'mean_cot_estu', 'nro_cot_bod',
       'nro_cot_depto', 'nro_cot_esta', 'nro_cot_estu', 'nro_proyectos',
       'sexo', 'tipo_cliente', 'Altos del Valle', 'Edificio Urban 1470',
       'San Andres Del Valle', 'Edificio Mil610', 'Edificio Junge',
       'profesion'],
      dtype='object'), 'personas_dummies_shape': (6184, 173), 'personas_negocio': (1101,), 'personas_nonegocio': (5083,), 'x_train_shape': (4328, 173), 'y_train_shape': (4328,), 'x_test_shape': (1856, 173), 'y_test_shape': (1856,), 'model': {'PRT_BLR': {'conf_matrix': array([[1323,  198],
       [  77,  258]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.94      0.87      0.91      1521\n       True       0.57      0.77      0.65       335\n\navg / total       0.88      0.85      0.86      1856\n'}, 'KNN': {'name': 'KNN', 'best_estimator': KNeighborsClassifier(algorithm='auto', leaf_size=30, metric='minkowski',
           metric_params=None, n_jobs=1, n_neighbors=9, p=2,
           weights='uniform'), 'best_score': 0.8741914618369987, 'conf_matrix': array([[1471,   50],
       [ 170,  165]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.90      0.97      0.93      1521\n       True       0.77      0.49      0.60       335\n\navg / total       0.87      0.88      0.87      1856\n'}, 'PCAKNN': {'name': 'PCAKNN', 'best_estimator': LogisticRegression(C=0.4393970560760795, class_weight=None, dual=False,
          fit_intercept=True, intercept_scaling=1, max_iter=100,
          multi_class='ovr', n_jobs=1, penalty='l1', random_state=None,
          solver='liblinear', tol=0.0001, verbose=0, warm_start=False), 'best_score': 0.8923027166882277, 'conf_matrix': array([[1463,   58],
       [ 155,  180]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.90      0.96      0.93      1521\n       True       0.76      0.54      0.63       335\n\navg / total       0.88      0.89      0.88      1856\n'}, 'ULR': {'conf_matrix': array([[1461,   60],
       [ 151,  184]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.91      0.96      0.93      1521\n       True       0.75      0.55      0.64       335\n\navg / total       0.88      0.89      0.88      1856\n'}, 'BLR': {'conf_matrix': array([[1328,  193],
       [  79,  256]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.94      0.87      0.91      1521\n       True       0.57      0.76      0.65       335\n\navg / total       0.88      0.85      0.86      1856\n'}, 'UDT': {'name': 'UDT', 'best_estimator': DecisionTreeClassifier(class_weight=None, criterion='gini', max_depth=19,
            max_features=28, max_leaf_nodes=None,
            min_impurity_decrease=0.0, min_impurity_split=None,
            min_samples_leaf=4, min_samples_split=2,
            min_weight_fraction_leaf=0.0, presort=False, random_state=None,
            splitter='best'), 'best_score': 0.8879366106080206, 'conf_matrix': array([[1437,   84],
       [ 153,  182]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.90      0.94      0.92      1521\n       True       0.68      0.54      0.61       335\n\navg / total       0.86      0.87      0.87      1856\n'}, 'URF': {'name': 'URF', 'best_estimator': RandomForestClassifier(bootstrap=True, class_weight=None, criterion='gini',
            max_depth=23, max_features='sqrt', max_leaf_nodes=None,
            min_impurity_decrease=0.0, min_impurity_split=None,
            min_samples_leaf=1, min_samples_split=3,
            min_weight_fraction_leaf=0.0, n_estimators=70, n_jobs=1,
            oob_score=False, random_state=None, verbose=0,
            warm_start=False), 'best_score': 0.8929495472186287, 'conf_matrix': array([[1469,   52],
       [ 149,  186]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.91      0.97      0.94      1521\n       True       0.78      0.56      0.65       335\n\navg / total       0.89      0.89      0.88      1856\n'}, 'BRF': {'conf_matrix': array([[1328,  193],
       [  79,  256]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.94      0.87      0.91      1521\n       True       0.57      0.76      0.65       335\n\navg / total       0.88      0.85      0.86      1856\n'}, 'SVM': {'name': 'SVM', 'best_estimator': SVC(C=1, cache_size=200, class_weight='balanced', coef0=0.0,
  decision_function_shape='ovr', degree=3, gamma=0.1, kernel='rbf',
  max_iter=-1, probability=False, random_state=None, shrinking=True,
  tol=0.001, verbose=False), 'best_score': 0.8581824062095731, 'conf_matrix': array([[1366,  155],
       [  96,  239]], dtype=int64), 'cla_report': '             precision    recall  f1-score   support\n\n      False       0.93      0.90      0.92      1521\n       True       0.61      0.71      0.66       335\n\navg / total       0.88      0.86      0.87      1856\n'}}, 'time_exec': 1805.843022108078}}